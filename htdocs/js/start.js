...
(function()
{
    ...
    StartPage.init = function()
    {
        StartPage.ui.init();
        StartPage.auth.init();
        StartPage.register.init();
        StartPage.support.init();
        StartPage.spin.init()
    };
    StartPage.ui = {
        init: function()
        {
            StartPage.ui.initTopLink();
            StartPage.ui.initExpanders();
            StartPage.ui.initMenu();
            StartPage.ui.initScreenshots();
            StartPage.ui.initColorBox();
            StartPage.ui.initOrnaments();
            StartPage.ui.initCountdown()
        },
        initCountdown: function()
        {
            var $countdown = $('#countdown');
            if (!$countdown.length) return;
            var starting_seconds = $countdown.data('remaining'),
        ...
        initOrnaments: function()
        {
            ...
            {
                var $minHeight = 1500,
                    $ornamentsMiddle = $('.ornament-middle-right, .ornament-middle-left');
                if ($(this).height() > $minHeight)
                {
                    $($ornamentsMiddle, this).addClass('is-visible')
                }
                else $($ornamentsMiddle, this).addClass('is-hidden')
            })
        },
        initTopLink: function()
        {
            ...
            $(window).scroll(function()
            {
                ...
                setTimeout(function()
                {
                    ...
                    if (scroll > 100)
                    {
                        $top.removeClass("transparent").addClass("fade-in")
                    }
                    else $top.removeClass("fade-in").addClass("transparent")
                }, 100)
            })
        },
        ...
        initScreenshots: function()
        {
            var sliderAnimation = false,
                updateArrows = function()
                {
                    ...
                    left = parseInt(left);
                    var sliderWidth = $('.slider-secondary').css('width');
                    sliderWidth = sliderWidth.replace('px', '');
                    sliderWidth = parseInt(sliderWidth);
                    if (left >= 0)
                    {
                        $('.slider-arrow-prev').css("visibility", "hidden")
                    }
                    else $('.slider-arrow-prev').css('visibility', 'visible');
                    left = Math.abs(left);
                    if (left + sliderWidth >= 1716)
                    {
                        $('.slider-arrow-next').css('visibility', 'hidden')
                    }
                    else $('.slider-arrow-next').css('visibility', 'visible')
                };
            $('.slider-arrow-next').click(function()
            {
                if (sliderAnimation === false)
                {
            ...
            $("#page3").responsiveSlides(
            {
                ...
                maxwidth: 638,
                namespace: "pc",
                navContainer: "#pagination3"
            })
        },
        initColorBox: function()
        {
            $("a.screenshot-open").colorbox(
            {
                opacity: 0.6,
                maxWidth: '90%',
                maxHeight: '90%'
            ...
            var $cb = $('#colorbox');
            $cb.hammer().on("swipeleft", function()
            {
                $.colorbox.next()
            });
            $cb.hammer().on("swiperight", function()
            {
                $.colorbox.prev()
            })
        }
    ...
    StartPage.auth = {
        ...
        init: function()
        {
            $('#login').on('submit', StartPage.auth.submitLogin);
            $('#world-select').on('click', '.world-select-show-all', StartPage.auth.showMoreWorlds)
        },
        submitLogin: function(e)
        {
            e.preventDefault();
            if (StartPage.auth.busy) return false;
            StartPage.auth.busy = true;
            var $login_button = $('.btn-login'),
                original_text = $login_button.html();
            $login_button.text('Working...');
            var $form = $('#login'),
            ...
            }, function(result)
            {
                if (result.error)
                {
                    StartPage.auth.busy = false;
                    $login_button.html(original_text);
                    StartPage.noticeBox.show(result.error);
                    GAPageTracking.track(
                    {
                        event: 'event',
                        eventCategory: 'Form Tracking ',
                        eventAction: 'form (login_form)',
                        eventLabel: 'error_message|user_or_pass_wrong'
                    });
                    return
                };
        ...
        showMoreWorlds: function()
        {
            $('.world-select-show-all').remove();
            $('.world-select .hidden').removeClass('hidden')
        }
    };
    StartPage.register = {
        ...
        init: function()
        {
            $('.require-validation').change(this.validateInputField);
            $('.message-suggestion-container').on('click', '.suggested-name', this.handleClickOnSuggestedName);
            $('#register').on('submit', StartPage.register.doRegistration);
            $('#register-button').on('click', function(e)
            {
                e.preventDefault();
                $('#register').submit()
            });
            var $korea_age_check = $('#init-age-check');
            if ($korea_age_check.length)
            {
                StartPage.register.korea = true;
                $korea_age_check.click(KoreaAge.onInitAgeCheck)
            }
        },
        doRegistration: function(e)
        {
            e.preventDefault();
            if (StartPage.register.korea && !KoreaAge.isAgeVerified())
            {
                StartPage.noticeBox.show('??/?? ?? ??');
                return false
            };
            if (StartPage.register.busy) return false;
            StartPage.register.busy = true;
            var $register_button = $('#register-button'),
                original_text = $register_button.html();
            $register_button.find('span').text('Working...');
            var data = $(this).serializeArray();
            $.post(StartPage.uris.register, data, function(result)
            {
                StartPage.register.busy = false;
                $register_button.html(original_text);
                if (result.error)
                {
                    StartPage.noticeBox.show(result.error);
                    return
                ...
                if (result.errors)
                {
                    ...
                    StartPage.noticeBox.show(error_array.join("<br />"));
                    return
                };
        ...
        validateInputField: function()
        {
            var $this = $(this),
                type = $this.data('type'),
                value = $this.val();
            $.post(StartPage.uris.register_validate,
            {
                type: type,
                value: value
            }, StartPage.register.handleInputFieldValidationResult)
        },
        handleInputFieldValidationResult: function(result)
        {
            var $form_element = $('#form-element-' + result.type);
            if (result.status === 'OK')
            {
                StartPage.register.setInputFieldValid($form_element)
            }
            else
            {
                StartPage.register.setInputFieldInvalid($form_element, result.errors);
                if (result.suggestions) StartPage.register.handleNameSuggestions(result.suggestions)
            }
        },
        setInputFieldValid: function($form_element)
        {
            var $error_container = $form_element.find('.error-message'),
                $validation_indicator = $form_element.find('.validation');
            $error_container.hide();
            $validation_indicator.attr('class', 'validation validation-true');
            var $next_error = $('#register').find('.validation-false');
            if ($next_error.length) $next_error.eq(0).parents('.form-element').find('.error-message').show()
        },
        setInputFieldInvalid: function($form_element, errors)
        {
            var $error_container = $form_element.find('.error-message'),
                $validation_indicator = $form_element.find('.validation');
            $validation_indicator.attr('class', 'validation validation-false');
            if (StartPage.register.numberOfErrors() === 1) $error_container.show();
            var $error_message = $error_container.find('.message'),
                error_strings = [];
            $.each(errors, function(i, error)
            {
                ...
                GAPageTracking.track(
                {
                    event: 'event',
                    eventCategory: 'Form Tracking ',
                    eventAction: 'form (register_form)',
                    eventLabel: 'error_message|' + i
                })
            });
            $error_message.html(error_strings.join("<br />"))
        },
        numberOfErrors: function()
        {
            return $('#register').find('.validation-false').length
        },
        handleNameSuggestions: function(data)
        {
            var register = StartPage.register;
            register.nameSuggestions = [];
            $.each(data, function(key, suggested_name)
            {
                var link = '<a href="#" class="suggested-name" data-name="' + suggested_name + '">' + suggested_name + '</a>';
                register.nameSuggestions.push(link)
            });
            if (register.nameSuggestions.length === 0)
            {
                register.hideNameSuggestions()
            }
            else register.showNameSuggestions(0)
        },
        showNameSuggestions: function(page)
        {
            var $container = $('.message-suggestion-container').show(),
                $ul = $container.find('ul').html(''),
                per_page = StartPage.register.SUGGESTIONS_PER_PAGE,
                page_start = page * per_page,
                page_end = page_start + per_page;
            $.each(StartPage.register.nameSuggestions.slice(page_start, page_end), function(i, link)
            {
                $ul.append('<li>' + link + '</li>')
            });
            $('#message-suggestion-more-link').off('click').on('click', function()
            {
                var new_page = (page + 2) * per_page > StartPage.register.nameSuggestions.length ? 0 : page + 1;
                StartPage.register.showNameSuggestions(new_page)
            })
        },
        hideNameSuggestions: function()
        {
            $('.message-suggestion-container').hide()
        },
        handleClickOnSuggestedName: function()
        {
            $('#register').find('input[name=register_username]').val($(this).data('name')).trigger('change')
        }
    };
    StartPage.noticeBox = {
        ...
        {
            if (!type) type = 'error';
            $('.auto-hide-box').remove();
            var fade_out_time = 2e3,
                container = $('body');
            $("<div/>",
            {
                "class": 'auto-hide-box ' + type + '-box',
                click: function()
                {
                    $(this).remove()
                },
                html: "<i class=\"icon-" + type + "\"></i> <p>" + message + "</p>"
    ...
    StartPage.support = {
        init: function()
        {
            ...
            $('#headerlink-support').on('click', function()
            {
                $.getJSON('/page/support', function(r)
                {
                    StartPage.support.showSupport(r.link + '&inline=1')
                });
                return false
            })
        },
        showSupport: function(url)
        {
            ...
            var $win = $(window),
                ...
                $body = $('body'),
                support_width = Math.min(1100, win_width),
                support_height = Math.min(700, win_height);
            $body.append('<div class="fader"></div>');
            var $support_window = $('<div class="support-window" />'),
            ...
            $support_window.css('margin-left', -(support_width / 2) + 'px');
            $support_window.append($iframe);
            $body.append($support_window);
            $('.fader').on('click', StartPage.support.closeSupport);
            $(window).off('message').on('message', function(e)
            {
                if (e.originalEvent.data === 'support_window_close') StartPage.support.closeSupport()
            })
        },
        closeSupport: function()
        {
            $('.fader, .support-window').remove()
        }
    };
    StartPage.spin = {
        input: '',
        ...
        init: function()
        {
            ...
            {
                StartPage.spin.input += e.keyCode;
                if (StartPage.spin.input.length > StartPage.spin.pattern.length) StartPage.spin.input = StartPage.spin.input.substring(StartPage.spin.input.length - StartPage.spin.pattern.length);
                if (StartPage.spin.input === StartPage.spin.pattern) $('.l-header-content').addClass('spin')
            })
        }
    };
    $(function()
...
! function($)
{
    $.fn.shorten = function(e)
    {
        ...
        return e && $.extend(s, e), $(this).data("jquery.shorten") && !s.force ? !1 : ($(this).data("jquery.shorten", !0), $(document).off("click", ".post-read-expand"), $(document).on(
        {
            click: function()
            {
                var e = $(this);
                return e.hasClass("less") ? (e.removeClass("less"), e.html(s.moreText), e.parent().prev().prev().show(), e.parent().prev().hide()) : (e.addClass("less"), e.html(s.lessText), e.parent().prev().prev().hide(), e.parent().prev().show()), !1
            }
        }, ".post-read-expand"), this.each(function()
        {
            var e = $(this),
                t = e.html(),
                r = e.text().length;
            if (r > s.showChars)
            {
                var a = t.substr(0, s.showChars);
                if (a.indexOf("<") >= 0)
                {
                    for (var n = !1, l = "", h = 0, o = [], i = null, p = 0, c = 0; c <= s.showChars; p++)
                        if ("<" != t[p] || n || (n = !0, i = t.substring(p + 1, t.indexOf(">", p)), "/" == i[0] ? i != "/" + o[0] ? s.errMsg = "ERROR en HTML: the top of the stack should be the tag that closes" : o.shift() : "br" != i.toLowerCase() && o.unshift(i)), n && ">" == t[p] && (n = !1), n) l += t.charAt(p);
                        else if (c++, h <= s.showChars) l += t.charAt(p), h++;
                    else if (o.length > 0)
                    {
                        for (j = 0; j < o.length; j++) l += "</" + o[j] + ">";
                        break
                    }
                    a = $("<div/>").html(l + '<span class="ellip">' + s.ellipsesText + "</span>").html()
                }
                else a += s.ellipsesText;
                var d = '<div class="shortcontent">' + a + '</div><div class="allcontent">' + t + '</div><span><a href="javascript://nop/" class="post-read post-read-expand l-align-right">' + s.moreText + "</a></span>";
                e.html(d), e.find(".allcontent").hide(), $(".shortcontent p:last", e).css("margin-bottom", 0)
            }
        }))
    }
}(jQuery);;
/**** vendor/jquery.placeholder-enhanced.min.js ****/
...
(function(e, q)
{
    var g = "placeholder" in q.createElement("input") && "placeholder" in q.createElement("textarea"),
        ...
        t = function(c)
        {
            for (var d = ["placeholder", "name", "id"], f = {}, b, a = 0, s = c.attributes.length; a < s; a++) b = c.attributes[a], b.specified && 0 > e.inArray(b.name, d) && (f[b.name] = b.value);
            return f
        },
        p = function(c)
        {
    ...
    if (!g || k.normalize) g ? g && k.normalize && (e.fn.val = function()
    {
        var c = arguments;
        if (this[0]) return c.length ? this.each(function(d, f)
        {
            var b = e(f),
    ...
    }) : e.fn.val = function()
    {
        var c = arguments,
            d = this[0],
            f;
        if (d) return c.length ? this.each(function(b, a)
        {
            var d = e(a),
                f = e.data(a, "placeholderEnhanced"),
                g;
            g = d.attr("placeholder");
            f && g && l(a) ? c[0] || a.value === g ? c[0] && (d.hasClass(f.cssClass) && d.removeClass(f.cssClass), h.apply(d, c)) : a.value = d.addClass(f.cssClass).attr("placeholder") : h.apply(d, c)
        }) : (f = e(d).attr("placeholder")) && l(d) ? d.value === f ? "" : d.value : h.apply(this, c)
    }, e.fn.placeholderEnhanced = function(c)
    {
        var d = e.extend(k, c);
        if (this.length && (!g || d.normalize)) return "destroy" === c ? this.filter(function(d, b)
        {
        ...
        }).removeClass(d.cssClass).each(function(d, b)
        {
            ...
            g ? delete b._placeholder : (b.value === m && (b.value = ""), c && (p(a), a.prev().unbind(".placeholder").remove()));
            e.fn.val = h;
        ...
        }) : this.each(function(c, b)
        {
            if (!e.data(b, "placeholderEnhanced"))
            {
                ...
                var a = e(b),
                    ...
                    l = null,
                    n = null;
                g ? g && d.normalize && (b._placeholder = a.attr("placeholder"), l = function()
                {
                ...
                {
                    a.val() ? a.val() && h && n() : h ? (p(k), r(a)) : a.val(m).addClass(d.cssClass)
                }, h ? (n = function()
                {
                    p(a);
                    r(k)
                ...
                {
                    a.hasClass(d.cssClass) && (b.value = "", a.removeClass(d.cssClass))
                });
...
/**** start/KoreaAge.js ****/
var KoreaAge = {
    cookie_key: 'korea_age_birthday',
    isAgeVerified: function()
    {
        return $.cookie(KoreaAge.cookie_key)
    },
    openDanalPopup: function()
    {
        var danalPopup = window.open('/korea_age/index.php', 'danal_popup', 'status=0, width=500, height=550');
        danalPopup.focus()
    },
    onRegisterClick: function()
    {
        if (KoreaAge.isAgeVerified())
        {
            return true
        }
        else
    ...
    onInitAgeCheck: function(event)
    {
        KoreaAge.openDanalPopup();
        $('#agecheck-status').removeClass('error').addClass('info').text('??/?? ?? ?? ?');
        var ssnPoll = KoreaAge.createPollFunction(KoreaAge.cookie_key, KoreaAge.onReceiveSSN, 1e3);
        ssnPoll();
        event.preventDefault()
    },
    onReceiveSSN: function()
    {
        $('#agecheck-status').removeClass('info').addClass('success').text('??/?? ?? ??')
    },
    initEventHandlers: function()
    {
        $('#register_button').click(KoreaAge.onRegisterClick);
        $('#init-age-check').click(KoreaAge.onInitAgeCheck)
    },
    createPollFunction: function(key, callback, ms)
    {
        var poll = function()
        {
            value = $.cookie(key);
